...
public class GuiceToHk2JITResolver implements JustInTimeInjectionResolver {
    ...
    public boolean justInTimeResolution(Injectee failedInjectionPoint) {
        ...
        if (binding == null && !(Logger.class.isAssignableFrom(implClass))) {
            return false;
        }
        ...
        AbstractActiveDescriptor guiceBean;
        if (Logger.class.isAssignableFrom(implClass)) {
            guiceBean = new GuiceLoggerHk2Bean(contracts, qualifiers, implClass,
                failedInjectionPoint.getInjecteeDescriptor().getImplementationClass());
        } else {
            guiceBean = new GuiceServiceHk2Bean(contracts, qualifiers, implClass, binding);
        }
